...
CLIENTEFFECT_REGISTER_BEGIN( PrecacheEffectFlamebow )
...
class C_FlamebowBolt : public C_BaseCombatCharacter
{
	DECLARE_CLASS( C_FlamebowBolt, C_BaseCombatCharacter );
	DECLARE_CLIENTCLASS();
public:
...
private:
	...
	C_FlamebowBolt( const C_FlamebowBolt & ); // not defined, not accessible

	Vector	m_vecLastOrigin;
	bool	m_bUpdated;
};

IMPLEMENT_CLIENTCLASS_DT( C_FlamebowBolt, DT_FlamebowBolt, CFlamebowBolt )
END_RECV_TABLE()

//-----------------------------------------------------------------------------
// Purpose: 
//-----------------------------------------------------------------------------
C_FlamebowBolt::C_FlamebowBolt( void )
{
}
...
// Input  : updateType - 
//-----------------------------------------------------------------------------
void C_FlamebowBolt::OnDataChanged( DataUpdateType_t updateType )
{
...
// Output : int
//-----------------------------------------------------------------------------
int C_FlamebowBolt::DrawModel( int flags )
{
...
// Purpose: 
//-----------------------------------------------------------------------------
void C_FlamebowBolt::ClientThink( void )
{
	m_bUpdated = false;
}
...
// Input  : &data - 
//-----------------------------------------------------------------------------
void FCrosshairLoadCallback( const CEffectData &data )
{
...
DECLARE_CLIENT_EFFECT( "FlamebowLoad", FCrosshairLoadCallback );
